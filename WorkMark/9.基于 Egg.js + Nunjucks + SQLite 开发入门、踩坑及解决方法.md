...
由于管理后台是vue项目生成的单页面，地址为localhost:6060/admin/xxx，cookie保存在path为"/"的路径下，导致管理后台无法正确获取和删除cookie

解决方法：  
在 Getcookie 和 Delcookie 方法中添加path属性，指定cookie的ptah，例如： 
```
document.cookie = `${cname} = null; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/admin/`;
```
